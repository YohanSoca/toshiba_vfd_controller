...
$(document).ready(function(){
	...
	const fwd_btn = document.getElementById('fwd');
	const bwd_btn = document.getElementById('bwd');
	...
	fwd_btn.addEventListener('click', function (){
		if(this.style.backgroundColor === 'red') {
			bwd_btn.style.backgroundColor = 'red';
			this.style.backgroundColor = 'green';
			return
		}
		if(this.style.backgroundColor !== 'red') {
	...
	bwd_btn.addEventListener('click', function (){
		if(this.style.backgroundColor === 'red') {
			fwd_btn.style.backgroundColor = 'red';
			this.style.backgroundColor = 'green';
			return
		}
		if(this.style.backgroundColor !== 'red') {
	...
	// Create knob element, 300 x 300 px in size.
				const knob = pureknob.createKnob(650, 800);
    ...
    setInterval(function () {
			count++;
			if(count > 5) main_title.textContent = "Server is down";
			$.get("/get_status", function (data, status) {
				...
				if(connected) {
					main_title.textContent = "Toshiba VFD";
				}
				...
				// alert(`speed:${speed}, is_on:${is_on}, direction: ${direction}`)
				knob.setValue(speed)
				if(connected) {
					main_title.textContent = "Toshiba VFD";
				}
